...
Windows 11 hosts with Hyper-V (for virtualization testing), Docker Desktop with WSL2 (for containerization with dynamic memory allocation to maximum 20GB RAM and all logical cores), and native development toolchains are evaluated.
...
\begin{itemize}
	...
	\item Memory consumption in host system in VM (only Windows guest) and container (only Docker Desktop on Windows host) environments
\end{itemize}
