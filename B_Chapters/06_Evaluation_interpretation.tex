...
All measurements were repeated 30 times to ensure statistical robustness.
...
The figure shows a clear performance hierarchy: native Windows toolchain switching via Visual Studio de- and reinstallation requires approximately 12.9 minutes, while containerized approaches complete in under 2 minutes---native containers on Linux at 37.5 seconds and Docker Desktop on Windows at 1.75 minutes.
...
Native Linux containers complete the image pull in 36.5 seconds with high consistency (standard deviation of 0.04 seconds), while Docker Desktop requires 102.7 seconds.
...
Total containerized toolchain switching achieves 37.5 seconds on native Linux and 105.0 seconds (1.75 minutes) on Docker Desktop, representing 95.1\% and 86.4\% improvements over the native Windows baseline respectively.
...
The total virtualized toolchain switching time of 218.2 seconds (3.6 minutes) is dominated by the template download operation (87\%), with VM boot contributing 13\% and registration being negligible.
VM template download requires approximately 3.2 minutes on average, with moderate variance reflecting network bandwidth fluctuations during transfer of large VHDX files over a 1GbE connection.
These measurements used a Linux VM template of 20.5 GB; VM template size significantly impacts the total switching time.
VM registration is nearly instantaneous at 0.51 seconds, as Hyper-V efficiently handles configuration metadata operations.
...
Native Linux development tools installation averages 114.5 seconds with notably higher variance (standard deviation of 22.99 seconds) compared to other operations.
This variance stems from package manager behavior, including dependency resolution, mirror selection, and download speeds.
The skewed distribution (median of 109s vs.\ mean of 114.5s) indicates occasional slow runs that pull the average upward.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|}
        ...
        Native Linux (devtools) & 114.47 & 109.00 & 22.99 & 90.00 & 174.00 & $-$85.2\% \\
        Hyper-V VM & 218.18 & 217.48 & 9.99 & 200.53 & 242.73 & $-$71.7\% \\
        Docker Desktop (WSL2) & 105.00 & 100.78 & 10.02 & 93.02 & 126.50 & $-$86.4\% \\
...
The results establish a clear performance hierarchy: native containers on Linux complete switching in 37.5 seconds (95.1\% faster than baseline), followed by Docker Desktop on Windows at 1.75 minutes (86.4\% faster), native Linux package installation at 1.9 minutes (85.2\% faster), Hyper-V VM switching at 3.6 minutes (71.7\% faster), and native Windows Visual Studio switching at 12.9 minutes (baseline).
...
All benchmarks were executed 30 times per environment to ensure statistical robustness, with measurements focusing on wall-clock time, CPU utilization, and memory consumption.
...
Docker on Linux uses native containers without a hypervisor and therefor shows no host memory overhead in a similar manner.
...
Debian native and Docker on Linux achieve nearly identical results (10.9\% and 10.7\% faster respectively), demonstrating that containerization imposes negligible overhead when host and container share the same kernel.
...
Docker Desktop on Windows supports two primary methods for providing source code access to containers: bind mounts and named volumes.
Bind mounts directly expose host filesystem paths to containers, while named volumes store data within Docker's managed storage layer.
On native Linux, bind mounts perform efficiently since the container and host share the same filesystem layer.
On Windows, however, bind mounts require cross-filesystem translation between NTFS and the Linux container environment.
The benchmarks for Docker Desktop presented in preceding sections used volume-mounted source code; this section documents why bind mounts were excluded from the Windows-based Docker Desktop evaluation.

Initial testing revealed that Docker Desktop with bind mounts exhibits severe performance degradation compared to volume mounts, confirming findings reported by \citet{dockerDockerDesktopWSL2020,cncfDockerMacOSSlow2023}.
A single exploratory benchmark run quantified this penalty, with results presented in Table~\ref{tab:bind_mount_comparison} and Figure~\ref{fig:bind_mount_build_comparison}.
...
CPU delta rises from near-idle levels (below 1.5\%) to approximately 5.4--5.5\% on both platforms.
Peak memory delta increases substantially: Windows parallel execution requires 3.8 GB additional memory compared to negligible overhead for sequential runs, while Debian parallel execution shows a median increase of 845 MB with outliers reaching several gigabytes.
...
Linux-based environments achieve 24--28$\times$ speedup factors, reducing build times from approximately 15--19 minutes to under 45 seconds.
...
Containerized approaches reduced this considerably: native Linux containers completed switching in 37.5 seconds (95.1\% improvement), while Docker Desktop on Windows achieved 1.75 minutes (86.4\% improvement).
Hyper-V virtualization fell between these at 3.6 minutes (71.7\% improvement).
...
Compiler caching amplified these differences, with Linux environments achieving 24--28$\times$ speedups compared to 4--5$\times$ on Windows.
...
Containerization on native Linux imposes negligible overhead for both CPU-bound and I/O-bound workloads.
...
The combination of containerized Linux environments with compiler caching offers the largest performance gains, reducing build times by over 96\% compared to uncached Windows native builds.
